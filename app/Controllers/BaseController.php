...
abstract class BaseController extends Controller
{
    ...
    public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger)
    {
        ...
        // Check session for all controllers except Auth controller
        $currentController = $request->getUri()->getSegment(1);
        if ($currentController !== 'auth' && $currentController !== 'login') {
            $session = session();
